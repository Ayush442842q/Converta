...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Convert documents offline with Converta.")
    parser.add_argument("input", type=Path, help="Path to the source document.")
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        help="Path to the output file. Defaults to the input filename with the requested extension.",
    )
    parser.add_argument(
        "-t",
        "--to",
        default="pdf",
        help="Output extension to use when --output is not provided. Defaults to pdf.",
    )
...
def render_markdown(markdown_text: str, title: str) -> str:
    ...
    return HTML_TEMPLATE.format(title=escape(title), content=html_body)
...
def main() -> None:
    ...
    destination = args.output.resolve() if args.output else source.with_suffix(f".{args.to.lstrip('.')}")
    result = converter.convert(source, destination)
    print(f"Created {result.destination} using {result.engine}")
